...
const getStaffForEventType = async (eventTypeId) => {
	...
	const getMembers =
		POLICY_HANDLERS[eventType.staffPolicy] ?? POLICY_HANDLERS.any
...
const useOrgFiltering = ({
	...
	useEffect(() => {
		...
		const loadStaff = async () => {
			...
			try {
				...
				const isStaffInList = (s: OrgStaffMember): boolean =>
	...
	const filteredStaff = eventTypeStaff
		? allStaff.filter((s) => {
				const isInEventTypeStaff = (es: OrgStaffMember): boolean =>
					es.id === s.id
...
const loading =
	orgLoading || scheduleLoading || bookingsLoading || filtering.loading

...
const toBookingSlot = (timezone, bufferAfter, booking, dateStart, dateEnd) => {
	const effectiveStart =
		booking.startAt < dateStart ? dateStart : booking.startAt
...
const resolveStaffTimezone = async ({
	staffId,
	orgId = null,
	locationId = null,
}) => {
...
const createDefaultSchedule = async (
	...
	const resolvedTz = orgId ? await getOrgTimezone(orgId) : timezone
...
const rotateTemplate = async ({
	...
	const resolvedTimezone = orgId ? await getOrgTimezone(orgId) : timezone
	...
	const yesterdayUtc = parseWallClockToUtc(
		`${yesterdayStr}T00:00:00`,
		resolvedTimezone,
	)
...
const putTemplateSchema = {
	...
	weeklyHours: {
		type: 'array',
		required: true,
		items: { type: 'object', properties: weeklyHourItemSchema },
	},
...
if (!orgId && !req.body.timezone) {
	return httpResponse(res, generalStatus.BAD_REQUEST, {
		errors: { timezone: 'required for personal schedules' },
	})
}
...
const run = async () => {
	...
	const result = await mongoose.connection.db
		...
		.updateMany({ orgId: { $ne: null } }, { $unset: { timezone: '' } })

	console.log(
		`Updated ${result.modifiedCount} org schedule templates (removed timezone field)`,
	)
	...
	console.log(
		`Remaining org templates with timezone: ${remaining} (should be 0)`,
	)

...
```typescript
import { mockBookingFields, mockBookingFormConfig } from './mock'
import type {
	BookingField,
	BookingFormConfig,
	CreateBookingFieldBody,
...
const getBookingFields = (
	...
	return bookingFieldsStore.filter(matchesOwner).filter(matchesEventType)
}
...
const getMergedBookingForm = (eventTypeId: string): MergedBookingForm => {
	...
	const orgFields = bookingFieldsStore.filter((f) => f.eventTypeId === null)
	const serviceFields = bookingFieldsStore.filter(
		(f) => f.eventTypeId === eventTypeId,
	)
...
const buildBaseFieldSchema = (
	...
	if (fieldName === 'phone') {
		return field.required
			? z.string().min(1, t('validation.contactRequired'))
			: z.string().optional()
	}
		...
		>
			<div>
				...
				<span className="text-muted-foreground ml-2 text-sm">
					(
					{t(
						`type${field.type.charAt(0).toUpperCase() + field.type.slice(1)}` as 'typeText',
					)}
					)
				</span>
				{field.required && (
